...
import { getUserProfile, getUserProfileById } from '@/src/services/user';
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    const fetchUserProfileAndPosts = async () => {
      try {
        if (userId) {
          const userProfileData = await getUserProfileById(accessTokenFromCookie, userId);
          setUserProfile(userProfileData?.data[0]);
